...
- **Beatmaps which overflow the integer limits for combo (over 65,535) or ScoreV1 (over 2,147,483,647) must have a Lazer Only flag visible on the beatmap's information page.** Due to integer limits, scores on affected maps can only be set correctly in osu!lazer, as values this high will cause unintended behavior in osu!stable.
